...
func (s *FabricaTestSuite) TestGeneratedFilesIncludeSPDXHeaders() {
	project := s.createProject("spdx-headers-test", "github.com/test/spdx", "file")
	...
	err = filepath.WalkDir(project.Dir, func(path string, d fs.DirEntry, walkErr error) error {
		if walkErr != nil {
			return walkErr
		}
		if d.IsDir() {
		...
		relPath, relErr := filepath.Rel(project.Dir, path)
		if relErr != nil {
			return relErr
		}

		isGeneratedGo := strings.HasSuffix(relPath, "_generated.go")
		hasGeneratedBanner := strings.Contains(content, "Code generated by Fabrica")
		if !isGeneratedGo && !hasGeneratedBanner {
			return nil
		}
	...
	s.Require().NoError(err)
	s.Require().Greater(checked, 0, "expected to inspect generated files")
	s.Require().Empty(missing, "generated files missing SPDX headers:\n%s", strings.Join(missing, "\n"))
}
